...
public class kontrol_transaksi extends koneksi{
    ...
    String sql = "SELECT * FROM minuman,detail_order WHERE minuman.id_minuman=detail_order.id_minuman AND detail_order.id_order =  '"+cari+"'";
        try {
            ...
            data[2]=rs.getString("nama_minuman");
    ...
    public ResultSet total (String total) {
        String sql = "SELECT SUM(masakan.harga*detail_order.keterangan) AS total FROM minuman,detail_order WHERE minuman.id_minuman ="
                + " detail_order.id_minuman AND detail_order.id_order = '"+total+"' GROUP BY detail_order.id_order";
        try {
            rs = st.executeQuery(sql);
        } catch (SQLException ex) {
            Logger.getLogger(kontrol_transaksi.class.getName()).log(Level.SEVERE, null, ex);
